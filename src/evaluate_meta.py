...
def main(args):
    ...
    if metadata.model == "anml":
        ...
        def get_test_slow_params(slow_params, fast_params=None):
        ...
        def get_test_fast_params(rng, fast_params, zero_weight=False, keep_bias=False):
    ...
    tmp_slow_params = jax.tree_map(
        jax.device_put, get_test_slow_params(state["slow_params"], state["fast_params"]),
    )
