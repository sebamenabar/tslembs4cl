...
class MetaLearningWrapper:
    def __init__(
        ...
        if val_slow_apply is None:
            val_slow_apply = slow_apply
        if val_fast_apply is None:
            val_fast_apply = fast_apply
        self.val_slow_apply = val_slow_apply
        self.val_fast_apply = val_fast_apply
    ...
    def __call__(
        ...
        if slow_apply is None:
            slow_apply = self.val_slow_apply
        if fast_apply is None:
            fast_apply = self.val_fast_apply
    ...
    def test(
        ...
        if self.__test_inner_loop is None:
            self.__test_inner_loop = jax.jit(
                lambda x, y, opt_state, inner_lr, slow_params, fast_params, slow_state, fast_state: self.inner_updates(
                    slow_apply(slow_params, slow_state, None, x, training=False)[0],
                    y,
                    inner_lr=inner_lr,
                    opt_state=opt_state,
                    # slow_params=slow_params,
        ...
        if evaluate:
            ...
            return fast_params, fast_state, opt_state
    ...
    def fast_apply_and_loss(
        ...
        outputs, state = fast_apply(fast_params, fast_state, rng, *x, training=training)
    ...
    def outer_loop(
        ...
        if (reset_fast_params_fn is not None) and (
            self.reset_before_outer_loop is False
        ):
            print("Resetting inside inner loop")
            fast_params = reset_fast_params_fn(
                rng_reset_params,
                spt_classes,
                fast_params,
            )
        elif self.reset_before_outer_loop is True:
            print("no resetting inside outer loop")
    ...
    def single_step(
        ...
        if reset_before_outer_loop is None:
            reset_before_outer_loop = self.reset_before_outer_loop
        ...
        bsz = bx_spt.shape[0]
        if rng is not None:
            rng_reset_fast_params, rng_step, rng_pre_spt, rng_pre_qry = split(rng, 4)
        ...
        if reset_before_outer_loop:
            print("resetting before outer loop")
            fast_params = reset_fast_params_fn(
                rng_reset_fast_params, spt_classes.reshape((-1,)), fast_params
            )
    ...
    def inner_loop(
        ...
        if rng is not None:
            rng_slow, rng_updates, rng_first, rng_last = split(rng, 4)
        else:
            rng_slow = rng_updates = rng_last = rng_first = None
        slow_outputs, slow_state = slow_apply(
            slow_params,
            slow_state,
            rng_slow,
...
class CLWrapper(MetaLearningWrapper):
    def inner_updates(
        ...
        carry, (losses, auxs) = jax.lax.scan(
            scan_fun,
            (fast_params, fast_state, opt_state),
            (*slow_outputs, y, rngs),
        )
